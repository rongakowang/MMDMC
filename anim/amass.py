...
def load(
    ...
    # y_axis = np.array([[0, 0, 1], [0, 1, 0], [-1, 0, 0]])
    # y = quat.from_xform(y_axis)
    # trans_quat = quat.mul(y, trans_quat)
    ...
    axangles = np.vstack((np.zeros((1, 156)), axangles[0].reshape(1, 156).repeat(rest_frames, axis=0), axangles))

    num_frames = len(axangles)
    axangles = axangles.reshape([num_frames, -1, 3])[:,:NUM_JOINTS]
    quats = quat.from_axis_angle(axangles)

    quats[0, 13, :] = quat.from_xform(np.array([[cos(-np.pi / 4), -sin(-np.pi /4), 0], [sin(-np.pi / 4), cos(-np.pi / 4), 0], [0, 0, 1]]))
    quats[0, 14, :] = quat.from_xform(np.array([[cos(np.pi / 4), -sin(np.pi /4), 0], [sin(np.pi / 4), cos(np.pi / 4), 0], [0, 0, 1]]))

    quat_at_first_frame = quats[1, 0]
    rot_cali = quat.inv(quat_at_first_frame)
    for i in range(1, quats.shape[0]):
        quats[i, 0] = quat.mul(rot_cali, quats[i, 0])
    ...
    amas_trans = np.vstack((amass_dict["trans"][0].reshape(1, 3).repeat(rest_frames + 1, axis=0), amass_dict["trans"]))
    # trans = ((amas_trans - amas_trans[0]) @ quat.to_xform(trans_quat).T) * scale + root_pos
    trans = ((amas_trans - amas_trans[0]) @ (quat.to_xform(rot_cali).T)) * scale + root_pos
