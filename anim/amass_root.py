...
def load(
    ...
    axangles = np.vstack((np.zeros((1, 156)), axangles))
    axangles = np.hstack((np.zeros((axangles.shape[0], 3)), axangles))
    
    num_frames = len(axangles)
    axangles = axangles.reshape([num_frames, -1, 3])[:,:NUM_JOINTS]
    quats = quat.from_axis_angle(axangles)
    root_rot = quat.mul(trans_quat[None], quats[:,1])
    quats[:,1] = root_rot

    if skel == None:
        # Load SMPL parmeters.
        smplh_dict = load_model(smplh_path, gender)
        parents = smplh_dict["kintree_table"][0]
        # parents += 1
        parents = np.insert(parents, 1, 0)
        parents[0] = -1
        parents[2:] += 1
        # for i, k in enumerate(names):
        #     if i > 0:
        #         print(k, names[parents[i]])
        ...
        J_positions = np.vstack((np.zeros((1, 3)), J_positions))

        # root_offset = J_positions[1]
        offsets = J_positions - J_positions[parents]

        offsets[0] = 0
        skel = Skel.from_names_parents_offsets(names, parents, offsets, skel_name="SMPLH")
    
    center_pos = skel.offsets[1][None].repeat(len(quats), axis=0)
    amas_trans = np.vstack((amass_dict["trans"][0], amass_dict["trans"]))
    trans = amas_trans * scale + center_pos
